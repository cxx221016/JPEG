...
struct Serial
{
    ...
    unsigned char yhencode(std::ifstream& fin,const std::string& filename);
    unsigned char yhdecode(std::ifstream& fin,const std::string& filename);
    Serial();
    ~Serial();
...
private:
    ...
    std::string filename;
public:
    JpegParser(const std::string& filename,bool flag=false):filename(filename),flag(flag),fin(filename,std::ios::binary){}
    ~JpegParser(){fin.close();}
    bool Parser();
    static bool isjpeg(std::ifstream& fin);
